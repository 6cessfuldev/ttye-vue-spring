...
export default {
  ...
  props: ['id'],
  setup() {
    // retrieve the article element using a ref
    const article = ref(null)

    // define the handleMove and handleOut functions
    function handleMove(e) {
      // get the coordinates and dimensions of the article element
      const { x, y, width, height } = article.value.getBoundingClientRect()
      const cx = x + width / 2
      const cy = y + height / 2

      // compute the distance between the cursor and the center of the card
      const { pageX, pageY } = e
      const dx = (cx - pageX) / (width / 2)
      const dy = (cy - pageY) / (height / 2)

      // rotate the card around the x and y axes
      article.value.style.transform = `rotateX(${10 * dy * -1}deg) rotateY(${10 * dx}deg)`
    }

    function handleOut() {
      // reset the transform property on mouseout
      article.value.style.transform = 'initial'
    }

    onMounted(() => {
      // retrieve the article element from the DOM
      article.value = document.querySelector('article')

      // add event listeners for mousemove and mouseout
      article.value.addEventListener('mousemove', handleMove)
      article.value.addEventListener('mouseout', handleOut)
    })

    onUnmounted(() => {
      // remove event listeners on unmount
      article.value.removeEventListener('mousemove', handleMove)
      article.value.removeEventListener('mouseout', handleOut)
    })
